...
export default function ElectricityBill() {
  ...
  return <section>
    <PageHeader number="4" title="Electricity Bill Calculator" description="Calculate the total bill using the rate assigned to the consumption tier." />
    <div className="workspace-card two-column">
      <form onSubmit={calculate} className="form-panel">
        ...
        <div className="button-row">
          <button className="primary" type="submit">Calculate Bill</button>
          <button className="secondary" type="button" onClick={clear}>Clear</button>
        </div>{error && <div className="feedback error">{error}</div>}
      </form>
      <div 
      className="result-panel bill-result">
        {result ? <>
        <span className={'status ' + (result.total >= 5000 ? 'danger' : 'success')}>{result.usage}</span>
        <h2>{result.customer}</h2>
        <div className="detail-row">
          <span>Consumption</span>
          <strong>{result.kwh.toLocaleString()} kWh</strong>
        </div>
        <div className="detail-row">
          <span>Rate Applied</span>
          <strong>₱{result.rate.toFixed(2)} / kWh</strong>
        </div>
        <div className="total-row">
          <span>Total Bill</span>
          <strong>₱{result.total.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</strong>
        </div>
        </> : 
        <div className="empty-state">
          <b>₱</b>
          <h3>Bill summary</h3>
          <p>Calculation details will appear here.</p>
        </div>}
      </div>
    </div>
  </section>;
