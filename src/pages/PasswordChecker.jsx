...
export default function PasswordChecker() {
  ...
  <div className="workspace-card two-column">
    <form onSubmit={check} className="form-panel">
        <label>Password
            <input type="password" value={password} 
            onChange={(e) => setPassword(e.target.value)} 
            placeholder="Enter a password" />
            </label>
            <p className="hint">Weak: under 6 • Medium: 6–9 • Strong: 10+</p>
            <div className="button-row">
              <button className="primary" type="submit">Check Password</button>
          ...
          <div className="result-panel">
            {result ? (
              <>
                <span className={'status ' + result.strength.toLowerCase()}>{result.strength} Password</span>
                <h2>Password Status</h2>
                <div className="strength-meter"><i className={result.strength.toLowerCase()}></i></div>
                <p>{result.message}</p>
                <small>{result.length} characters entered</small>
              </>
            ) : (
              <div className="empty-state">
                <b>⌁</b>
                <h3>Check your password</h3>
                <p>Your strength result will appear here.</p>
              </div>
            )}
          </div>
        </div>
